...
const ProWallet = ({ data }) => {
	const route = useRouter();
	const [detail, setDetail] = React.useState({
		gotPaid: data.gotPaid,
		payToApp: data.payToApp,
	...
	const handleSubmit = async () => {
		try {
			...
			await updateDoc(docRef, {
				wallet: {
					gotPaid: detail.gotPaid,
					payToApp: detail.payToApp,
					...
					>
						<Grid item xs={12} sm={12} ml={2} mr={2} pt={2}>
							<TextField
								...
								value={detail.totalEarned}
								onChange={(e) =>
									setDetail({ ...detail, totalEarned: e.target.value })
								}
							/>
						</Grid>
					</Grid>
					...
					>
						<Grid item xs={12} sm={6}>
							<TextField
								...
								sx={{ my: 1 }}
								inputProps={{ type: "number" }}
								value={detail.gotPaid}
								onChange={(e) =>
									setDetail({ ...detail, gotPaid: e.target.value })
								}
							/>
						</Grid>
						<Grid item xs={12} sm={6}>
							<TextField
								...
								inputProps={{ type: "number" }}
								sx={{ my: 1 }}
								value={detail.payToApp}
								onChange={(e) =>
									setDetail({ ...detail, payToApp: e.target.value })
								}
							/>
						</Grid>
					</Grid>
